...
- Re-pinned sibling package(s) `LMSupply.*` 0.79.1 -> 0.79.2.
...
### Changed
- Bumped `FluxImprover` 0.11.0 → 0.11.1 (dependency freshness, no known breaking changes consumed).
...
### Changed
- `LMSupply.Core`/`.Embedder`/`.Generator`/`.Captioner`/`.Ocr` dependencies raised `0.34.17` →
  `0.42.1` (dependency-freshness gate, no FileFlux API surface change).
