...
public class WordDocumentStyleBasedTests : IDisposable
{
    ...
    public async Task ExtractAsync_WithHeadingStyles_ShouldRecognizeStyleBasedSections()
    {
        ...
        // Heading 스타일 감지 검증
        if (result.StructuralHints.TryGetValue("heading_styles_detected", out object? headingValue))
        {
        ...
        // 섹션 구조 검증
        if (result.StructuralHints.TryGetValue("section_count", out object? sectionValue))
        {
    ...
    public async Task ExtractAsync_WithTablesAndCaptions_ShouldExtractTableContext()
    {
        ...
        // 표 관련 구조적 힌트 검증
        if (result.StructuralHints.TryGetValue("table_count", out object? value))
        {
        ...
        // 캡션 연결 검증 (향후 구현 예정)
        if (result.StructuralHints.TryGetValue("has_table_captions", out object? captionValue))
        {
    ...
    public async Task ExtractAsync_WithComplexFormatting_ShouldPreserveImportanceScoring()
    {
        ...
        // 서식 기반 중요도 분석 검증
        if (result.StructuralHints.TryGetValue("formatting_analysis", out object? analysisValue))
        {
    ...
    public async Task ExtractAsync_WithFootnotesAndEndnotes_ShouldPreserveReferences()
    {
        ...
        // 각주/미주 관련 힌트 검증
        if (result.StructuralHints.TryGetValue("has_footnotes", out object? value))
        {
            Assert.True((bool)value);
            _output.WriteLine("✅ 각주 감지");
        }

        if (result.StructuralHints.TryGetValue("footnote_count", out object? footnoteValue))
        {
