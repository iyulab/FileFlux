...
public class MultiModalPowerPointDocumentReader : IDocumentReader
{
    ...
    public async Task<RawContent> ExtractAsync(string filePath, CancellationToken cancellationToken = default)
    {
        ...
        var baseContent = await _basePowerPointReader.ExtractAsync(filePath, cancellationToken);
    ...
    {
        // 기본 PowerPoint 텍스트 추출
        var baseContent = await _basePowerPointReader.ExtractAsync(stream, fileName, cancellationToken);
