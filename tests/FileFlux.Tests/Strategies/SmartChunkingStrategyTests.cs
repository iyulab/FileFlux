...
Integration with existing systems is straightforward and well-documented. The API provides flexible configuration options. Developers can customize chunking behavior to meet specific requirements.",
        ...
        {
            _output.WriteLine($"Chunk {chunk.Index}:");
            ...
            Assert.True(chunk.Props.ContainsKey("Completeness"));
            var reportedCompleteness = Convert.ToDouble(chunk.Props["Completeness"]);
            
            // Verify at least 70% completeness
            Assert.True(reportedCompleteness >= 0.7, 
                $"Chunk {chunk.Index} has completeness {reportedCompleteness:P0}, expected >= 70%");
    ...
    public async Task ChunkAsync_HandlesOverlapCorrectly()
    {
        ...
        if (chunks.Count == 1)
        {
            ...
            Assert.True(chunks[0].Props.ContainsKey("HasOverlap"));
        ...
        for (int i = 1; i < chunks.Count; i++)
        {
            ...
            Assert.True(chunks[i].Props.ContainsKey("HasOverlap"));
            Assert.True(Convert.ToBoolean(chunks[i].Props["HasOverlap"]));
    ...
    public async Task ChunkAsync_HandlesLongSentencesGracefully()
    {
        ...
        foreach (var chunk in chunks)
        {
            ...
            Assert.True(chunk.Props.ContainsKey("SentenceIntegrity"));
        }
    }
...
Quality metrics are continuously monitored and improved. The system adapts to different document types automatically. Performance benchmarks exceed industry standards.",
        ...
        foreach (var chunk in chunks)
        {
            ...
            Assert.True(chunk.QualityScore() >= 0.6, 
                $"Chunk {chunk.Index} has low quality score: {chunk.QualityScore():F2}");
            
            // Verify quality components
            Assert.True(chunk.Props.ContainsKey("Completeness"));
            Assert.True(chunk.Props.ContainsKey("SemanticCoherence"));
            Assert.True(chunk.Props.ContainsKey("SentenceIntegrity"));
            
            var completeness = Convert.ToDouble(chunk.Props["Completeness"]);
            var coherence = Convert.ToDouble(chunk.Props["SemanticCoherence"]);
            var integrity = Convert.ToDouble(chunk.Props["SentenceIntegrity"]);
