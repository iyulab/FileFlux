...
public class LLMChunkFilterTests
{
    ...
    public async Task FilterChunksAsync_PreservesOrderWhenRequested()
    {
        ...
        {
            Assert.True(filtered[i].Chunk.Index > filtered[i - 1].Chunk.Index);
        }
    }
    ...
    public async Task FilterChunksAsync_HandlesQualityWeighting()
    {
        ...
        if (highQualityFiltered.Any(fc => fc.Chunk.Index == 0))
        {
            var detailedChunk = highQualityFiltered.First(fc => fc.Chunk.Index == 0);
