...
public class ExcelDocumentReaderTests
{
    ...
    {
        // Arrange
        var testFile = @"D:\data\FileFlux\tests\test-xlsx\file_example_XLS_100.xls";
        ...
        // FileInfo 검증
        Assert.Equal(Path.GetFileName(fileToTest), result.File.Name);
        Assert.Equal(".xlsx", result.File.Extension);
        Assert.Equal("ExcelReader", result.ReaderType);
        Assert.True(result.File.Size > 0);

        // StructuralHints 검증
        Assert.Equal("excel_workbook", result.Hints["file_type"]);
        Assert.True((int)result.Hints["character_count"] >= 0);
        Assert.True((int)result.Hints["worksheet_count"] >= 0);

        // 워크시트와 관련된 힌트들 확인
        if (result.Hints.TryGetValue("worksheet_count", out object? value))
        {
            var worksheetCount = (int)value;
            _logger.LogInformation("Worksheet count: {Count}", worksheetCount);
        }

        if (result.Hints.TryGetValue("total_rows", out object? rowValue))
        {
    ...
    public async Task ExtractAsync_WithStream_ShouldExtractContent()
    {
        ...
        try
        {
            ...
            Assert.NotNull(result);
            Assert.Equal("test.xlsx", result.File.Name);
            Assert.Equal(".xlsx", result.File.Extension);
            Assert.Equal("ExcelReader", result.ReaderType);
        }
        catch (Exception ex)
