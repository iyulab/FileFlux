...
/// assemblies next to this test (the FileFlux packages, the sibling packages they integrate and the
...
public class DocsSnippetRosterTests
{
    ...
    public void APhantomProperty_AndAPhantomType_AreReported()
    {
        ...
        findings.Should().BeEquivalentTo(["ChunkingOptions.NoSuchOption", "ChunkingOptions.Nested", "ImaginaryOptions (no such type in the library assemblies)"]);
    }

    [Fact]
    public void AGetOnlyCollection_AcceptsANestedInitializer_ButNotAnAssignment()
    {
        const string assigned = """
            var options = new ChunkingOptions
            {
                CustomProperties = new Dictionary<string, object> { ["a"] = 1 }
            };
            """;
        ...
        FindSnippets(assigned).SelectMany(s => Check(s, OptionTypes())).Should().BeEquivalentTo(["ChunkingOptions.CustomProperties"],
            "CustomProperties is get-only: assigning a new dictionary does not compile");
        FindSnippets(initialized).SelectMany(s => Check(s, OptionTypes())).Should().BeEmpty();
    }
    ...
    public void APhantomRegistrationCall_IsReported_AndARealOneIsNot()
    {
        ...
        var calls = RegistrationCalls(
            "services.AddFileFlux().AddLogging().UseImaginaryThing();\n" +
        ...
        calls.Should().BeEquivalentTo(["AddFileFlux", "AddLogging", "UseImaginaryThing", "NoSuchProcessAsync", "ToUpperInvariant"]);
    ...
    /// <summary>
    /// The assemblies whose public method names count as "exists": the FileFlux packages, the sibling
    ...
    [
        "FileFlux*.dll", "Flux.*.dll", "FluxImprover*.dll", "LMSupply*.dll", "Microsoft.Extensions.*.dll",
    ];
