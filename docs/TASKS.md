...
> 순수 RAG 전처리 SDK - 문서를 구조화된 청크로 변환하는 .NET 10 SDK
...
**현재 상태**: v0.4.1 - CLI 및 메타데이터 enrichment 기능 강화
**프로덕션 준비**: ✅ 완료 (테스트 100% 통과, AI 서비스 선택적 구현)
...
/src/
  /FileFlux/                # 통합 패키지
    /Core/                  # 인터페이스 (전처리 계약)
    /Domain/                # 도메인 모델 (청크, 메타데이터)
    /Infrastructure/        # 구현체 (파서, 전처리기)
  /FileFlux.CLI/            # 명령줄 인터페이스 도구
/tests/
  /FileFlux.Tests/          # 테스트
/samples/
...
**마지막 업데이트**: 2025-11-24 (CLI 기능 강화, 문서 정리)
**현재 버전**: v0.4.1 (프로덕션 준비 완료)
**개발 상태**: ✅ 안정화 완료 - 모든 핵심 기능 구현
